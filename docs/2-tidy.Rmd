...
# copy import files to tidy project

tidy_data_directory <- '~/actualize-tidy/data/'

import_data_directory <- '~/actualize-import/data/'
...
if (length(source_files) > 0) {
  warning('No files found in data directory')
}

# copy the files to the new folder

copy_sucess <- file.copy(
  from = source_files, 
  to = tidy_data_directory
)

if (any(!copy_sucess)) {
  warning('Some files not copied, check that the files exist')
}
